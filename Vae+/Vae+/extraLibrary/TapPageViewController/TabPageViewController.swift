...
public class TabPageViewController: UIPageViewController {
    public var isInfinity: Bool = false // 是否无穷多
    public var option: TabPageOption = TabPageOption()
    public var tabItems: [(viewController: UIViewController, title: String)] = [] { // 放入元组的数组
        didSet {
    ...
    var currentIndex: Int? { // 当前索引
        guard let viewController = viewControllers?.first else { // 判断是否存在控制器,,不存在则返回索引值为空
            return nil
        }
        return tabItems.map{ $0.viewController }.indexOf(viewController) // 返回当前控制器的索引
    }
    private var beforeIndex: Int = 0 // 上一个索引
    private var tabItemsCount = 0 // items数量(即控制器数量)
    private var defaultContentOffsetX: CGFloat { // 默认的 x 轴方向的偏移值
        return self.view.bounds.width
    }
    private var shouldScrollCurrentBar: Bool = true // 是否需要滚动当前的 bar
    lazy private var tabView: TabView = self.configuredTabView()

    public static func create() -> TabPageViewController { // 创建 TabPageViewController
    ...
    override public func viewDidLoad() {
        ...
        navigationItem.title = "hjkhsjkhkfjshjk"
